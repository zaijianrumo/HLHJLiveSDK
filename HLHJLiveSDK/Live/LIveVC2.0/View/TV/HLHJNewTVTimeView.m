...
@interface HLHJNewTVTimeView()

@property (nonatomic, assign) NSInteger selectedRow;

@end

@implementation HLHJNewTVTimeView
...
-(void)layoutSubviews {
    ...
    if (!_collectionView) {
        ...
        layout.itemSize = CGSizeMake((self.frame.size.width - 30)/7,  self.frame.size.height);
        ...
        [_collectionView registerClass:[HLHJNewTVTimeCollectionCell class] forCellWithReuseIdentifier:@"HLHJNewTVTimeCollectionCell"];
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    HLHJNewTVTimeCollectionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:@"HLHJNewTVTimeCollectionCell" forIndexPath:indexPath];

    NSArray *arr = @[@"周一",@"周二",@"周三",@"周四",@"周五",@"周六",@"周日"];
    
//    NSString *time = [self getWeekTime];
//    NSArray *b = [time componentsSeparatedByString:@","];
//    NSArray *datearr = [self getDayArrayLeftDate:b[2] rightDate:b[3] dateFormatter:@"MM-dd"];

    
    cell.timeLab.text =  [self getCurrentWeeksDateFormatter:@"dd"][indexPath.row];
    if (self.selectedRow == indexPath.row) {
        cell.timeLab.textColor = [TMEngineConfig instance].themeColor;
        cell.timeLab.font = [UIFont fontWithName:@"PingFang-SC-Medium" size:16];
    }else{
        cell.timeLab.textColor = [UIColor colorWithHexString:@"999999"];
        cell.timeLab.font = [UIFont fontWithName:@"PingFang-SC-Medium" size:14];
    }
    return cell;
}
